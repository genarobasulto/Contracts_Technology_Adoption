...
file_path = 'C:\Users\gbasulto\Box\Carnegie Mellon\AirLiquide\Data\Nyc_truck_data_long.csv'
...
trucking_data.sort_values(by = "n_trucks", inplace=True )

###################################
# Pareto Distribution Estimation. #
###################################

# Function to calculate ECDF for a group
def calculate_ecdf(group, mil_col):
    # Repeat low_mileage values by their weights (n_trucks), ensuring compatible data types
    repeated_values = np.repeat(group[mil_col].values, group["n_trucks"].values.astype(int))
    sorted_values = np.sort(repeated_values)  # Sort the repeated values
    ecdf = np.arange(1.0, len(sorted_values) + 1.0) / len(sorted_values)  # ECDF calculation
    return pd.DataFrame({mil_col: sorted_values, "ecdf_" + mil_col: ecdf}).drop_duplicates(mil_col)

# Apply ECDF calculation by region
low_ecdf = trucking_data.groupby('region').apply(calculate_ecdf, 'type_low').reset_index(level=0)
trucking_data = pd.merge(
    trucking_data,          # Original dataset
    low_ecdf,               # ECDF data
    on=["region", "type_low"],  # Merge keys
    how="left"              # Keep all rows from the original dataset
)
high_ecdf = trucking_data.groupby('region').apply(calculate_ecdf, 'type_high').reset_index(level=0)
trucking_data = pd.merge(
    trucking_data,          # Original dataset
    high_ecdf,               # ECDF data
    on=["region", "type_high"],  # Merge keys
    how="left"              # Keep all rows from the original dataset
)
mid_ecdf = trucking_data.groupby('region').apply(calculate_ecdf, 'type_mid').reset_index(level=0)
trucking_data = pd.merge(
    trucking_data,          # Original dataset
    mid_ecdf,               # ECDF data
    on=["region", "type_mid"],  # Merge keys
    how="left"              # Keep all rows from the original dataset
)

avg_ecdf = trucking_data.groupby('region').apply(calculate_ecdf, 'type_avg').reset_index(level=0)
trucking_data = pd.merge(
    trucking_data,          # Original dataset
    avg_ecdf,        # ECDF data
    on=["region", "type_avg"],  # Merge keys
    how="left"          # Keep all rows from the original dataset
)
...
alpha_by_region.to_csv("shape_parameters_5_regions_ny_long.csv", index = False)

# Plot Alphas by Region
plt.figure(figsize=(10, 6))
plt.plot(alpha_by_region['region'].astype(int), alpha_by_region['alpha'], label='Shape', marker='o', linestyle='-', color='blue')
plt.scatter(trucking_data['region'].astype(int), trucking_data['xm'], label='Scale', marker='x', linestyle='-', color='black')
...
plt.xlabel('Region')
plt.ylabel('Alpha')
plt.title('Pareto Parameters Estimates')
plt.legend()
plt.grid(True)
plt.show()
...
def plot_CDF(reg, how = "mid"):
    if how == "mid":
        vars_now = ['type_avg', 'ecdf_type_avg', 'estimated_cdf', 'xm']
        sales = 'type_avg'
    elif how == "lower":
        vars_now = ['type_low', 'ecdf_type_low', 'lower_estimated_cdf', 'xm_l']
        sales = 'type_low'
    elif how == "upper":
        vars_now = ['type_high', 'ecdf_type_high', 'upper_estimated_cdf', 'xm_u']
        sales = 'type_high'
